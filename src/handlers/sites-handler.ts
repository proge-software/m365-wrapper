...
export default class SitesHandler {
    ...
    public async getRootSite(): Promise<M365WrapperDataResult<Site>> {
        try {
            let item: Site = await this.client.api(`/sites/root`)
                .get();
    ...
    public async createFolder(siteId: string, parentItemId: string, folder: string | DriveItem): Promise<M365WrapperDataResult<DriveItem>> {
        try {
            ...
            let result: DriveItem = await this.client.api(`/sites/${siteId}/drive/items/${parentItemId}/children`)
                .post(driveItem);
